...
>
> This spec was the basis of v1.9.0, which was **yanked** the same day for two critical bugs (fictional Skill invocation contracts; spec's own Verification step 1 skipped) and one wrong design choice (pre-commit semantics where post-commit-with-revert was needed for the unattended use case).
>
> The successor spec, narrowed by baseline N=5, is **`docs/specs/2026-05-06-test-coverage-at-commit.md`** — one gate: tests-with-code.
>
> The evidence the new spec rests on is **`docs/specs/2026-05-06-ralph-loop-baseline.md`**.
>
> The body below is preserved as a historical record. Do not implement from it.

---

**Date:** 2026-05-06
**Status:** Superseded by `2026-05-06-test-coverage-at-commit.md`. Baseline showed only the test-coverage gap was real; DS / e2e / code-review gates were either undetermined or marginal over existing end-of-PR coverage.
**Owner:** Mat (CEO)
**Stance (original, preserved):** Land the smallest change that makes every atomic commit pass three gates. Add the outer loop only after the gates work.
